...
interface MyProfileListComponentProps {
  ...
  usingid: number;
  handleChangeProfile: (profile:UserTypes) => void;
}


export const MyProfileListComponent = ({ my_profiles,usingid,handleChangeProfile }: MyProfileListComponentProps) => {
  return(
    <div className="flex justify-center">
      <div className="flex my-3 gap-6 w-9/12 overflow-x-scroll pt-3 pb-5">
        {my_profiles.map((my_profile) => (
          <div key={my_profile.id} className={`min-w-20 h-20 rounded-3xl bg-${my_profile.color}-500 opacity-60 flex flex-col items-center justify-center ${my_profile.id == usingid && "border-2 border-neutral-100 opacity-100"}`} onClick={() => handleChangeProfile(my_profile)}>
